...
real*8 mu(2000,2000),mut(2000,2000,3),rz(2000),ry(2000),rs(2000),g(2000),ma(2000,2000),mat(2000,2000,3),ma0
real*8 refr_ind,f_mon,sum_g,sum_rs,mut_agg(2000,3),os(2000),rx(2000),x,d,cd,s,omega,gamma,sigma, wave, wave2, sig,sigma2,sigma1

pi=dacos(-1.d0)

open (1,file='CD.dat')
open (2,file='coordinates.dat')
open (3,file='absorption.dat')
...
write(*,*) "magnetic dipole moment amplitude (D)"
read (*,*) ma0
!ma0=0.d0
write(*,*) "bandwidths associated with longitudinal and transversal modes"
read(*,*) sigma1,sigma2
...
do i=1,n
   ...
   !!!ma0 is the magnetic dipole moment amplitude in D. m are unit vectors.
...
do i=1,n
   ...
   do j=i+1,n !!!upper triangle
      d=sqrt((rz(j)-rz(i))**2+(rx(j)-rx(i))**2+(ry(j)-ry(i))**2)!!!distance between i-th and j-th molecule
...
do i=1,n
   write(33,*) i,w(i) !!!eigenvalues of exciton hamiltonian
enddo


!compute magnetic and electric transition dipole moments
mut=0.d0
mat=0.d0
...
!compute rotational and oscillator strengths
!!!
...
sum_g=0
mut_agg=0
!write(*,*) 'rotational strengths , g factors & oscillator strength'
do i=1,n !!!excited states
   do j=1,n !!!molecules
...
if(j.eq.k) then !!!intrinsic chirality (intramolecular mu-m coupling)
...
do i=1,1000
   ...
   do j=1,n
      ...
      !!!impose different bandwidths for different states (narrow bandwidth for longitudinal modes, large for transversal; the state threshold depends on the specific input geometry)
      if(j.lt.2) sigma=sigma1
      if(j.ge.2) sigma=sigma2
      wave2=w(j)/1240 
      sig=sigma/1240
      cd=cd+w(j)*rs(j)*(1.d0/(sig*sqrt(pi)))*exp(-1.d0*((wave-wave2)/sig)**2) 

      
      s=s+w(j)*os(j)*(1.d0/(sig*sqrt(pi)))*exp(-1.d0*((wave-wave2)/sig)**2)
      !!!both cd and absorption in arbitrary units
   enddo
   write(1,*) omega, cd
   write(3,*) omega,s
enddo
